...
- [x] Wired up the OTLP ingestion side (`generate-e2e-env.ts`, `run-test.sh --env-file`) — and in doing so, **found a real bug**: sovdev-logger's own `OTEL_EXPORTER_OTLP_HEADERS` handling deviated from the actual OpenTelemetry spec and collided with the SDK's native env-var parsing whenever a header value contains `=` (any Basic Auth token). Traces flush crashed with `ERR_INVALID_HTTP_TOKEN`. Full diagnosis and fix landed in `fix/otlp-headers-spec-compliance` — see [`INVESTIGATE-otlp-headers-standard-compliance.md`](INVESTIGATE-otlp-headers-standard-compliance.md).
- [x] After that fix, hit a **second, different bug** with the same root class: `Authorization=Basic <token>` (the *new*, spec-correct header format) contains a space, and an unquoted value with a space gets word-split by bash's `source` just as badly as the old JSON did — the token was silently truncated to just `"Authorization=Basic"`, producing `401 "no credentials provided"` (not a bad token — no token at all). Fixed by quoting the value in `generate-e2e-env.ts`'s generated output. Full diagnosis in [`INVESTIGATE-grafana-cloud-otlp-data-loss.md`](INVESTIGATE-grafana-cloud-otlp-data-loss.md) (title kept for history — the actual cause wasn't data loss on Grafana's side, it was this quoting bug).
