...
- [ ] Create `PLAN-002-python-conformance.md` (prerequisite: PLAN-001, now complete) scoped to Option C — fix what `PLAN-001` already found (`response_json` dropped on 13/17 entries, `exception_message` trailing-space diff), re-run `compare-with-master.sh python` until it passes cleanly, and execute the promotion step to flip `README.md`
- [ ] Create `PLAN-003-spec-scaffolding-cleanup.md` (can run in parallel with PLAN-001/002) scoped to Option D — cut `llm-work-templates/` down to what's still needed once PLAN-001 exists, compress `07-anti-patterns.md` into a structured feature-mapping table, move the rest to `completed/` with a historical banner rather than deleting
- [ ] Create `PLAN-004-schema-driven-field-generation.md` (prerequisite: PLAN-001) scoped to Option E — generate per-language field-name constants from `specification/schemas/*.json`
