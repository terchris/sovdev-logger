...
8. **Write `{language}/README.md`** once the comparison passes, then update the root `README.md`'s status table to reflect the new "✅ Available" language — this promotion step is a required part of the language being done, not an afterthought. Verify every code example in the new README against the actual public API before publishing it; run the quickstart example for real.
